        ...
        <div class="col-12">
          ...
          <div *ngIf="!isLoading && !error">
            ...
            <div *ngIf="books.length > 0">
              <div class="row">
                <div class="col-lg-4 col-md-6 mb-4" *ngFor="let userBook of books">
                  <div class="card h-100">
                    <div class="card-body">
                      <div class="d-flex">
                        <img 
                          [src]="userBook.book.thumbnail || '/assets/images/book-placeholder.png'" 
                        ...
                        <div class="flex-grow-1">
                          ...
                          <div class="mb-2">
                            <span class="badge" [ngClass]="getStatusBadgeClass(userBook.status)">
                              {{ getStatusDisplayName(userBook.status) }}
                            </span>
                            <span *ngIf="userBook.isFavourite" class="badge bg-warning ms-1">
                              <i class="fas fa-heart"></i> Favorite
                            </span>
                          </div>
                          <div *ngIf="userBook.rating && userBook.rating > 0" class="mb-2">
                            <div class="text-warning">
                              <i *ngFor="let star of getStarArray(userBook.rating)" class="fas fa-star"></i>
                              <i *ngFor="let star of getEmptyStarArray(userBook.rating)" class="far fa-star"></i>
                            </div>
                          </div>
            ...
            <div *ngIf="books.length === 0" class="text-center py-5">
              ...
              <p class="text-muted">{{ userName }} hasn't added any books to their library yet.</p>
            </div>
          </div>
        </div>
      </div>
...
export class UserLibraryComponent implements OnInit, OnDestroy {
  ...
  loadUserLibrary(): void {
    ...
    this.libraryService.getUserLibrary(this.userId)
  ...
  calculateStats(): void {
    ...
    this.completedBooks = this.books.filter(book => book.status === 'read').length;
    this.currentlyReadingBooks = this.books.filter(book => book.status === 'currently_reading').length;
    this.wantToReadBooks = this.books.filter(book => book.status === 'to_read').length;
  }
